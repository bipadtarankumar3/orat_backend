<form class="pt-0" id="categoryForm">
    ...
    <!-- Title -->
    <div class="mb-3">
        <label class="form-label" for="ecommerce-size-title">Select Occution</label>
        <select name="occution_id" class="form-control" id="">
            <option value="">please Select</option>
            @foreach ($occution as $item)
            <option value="{{$item->id}}"  @if(isset($form_id)) @if($editData->parent_id == $item->id) selected @endif @endif>{{$item->title}}</option>
